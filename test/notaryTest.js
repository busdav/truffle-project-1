NotaryArtifact = artifacts.require("../contracts/Notary.sol");
...
contract("NotaryContract", function(accounts) {
  ...
  it('This is my testcase', function() {
    return NotaryArtifact.deployed().then(function(instance) {
      //here we can access the instance
      // console.log(instance);
    })
  });

  it('should have no entry for a test-hash', async function() {
    return NotaryArtifact.deployed().then(async function(instance) {
      try {
        ...
        assert.fail(true, true, 'Expected an exception, but test went through - check entrySet');
      } catch(error) {
        //test error message
